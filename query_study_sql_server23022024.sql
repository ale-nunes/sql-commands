...
-- AVG SALES AND SUM QUANTITY BY CUSTOMER
SELECT SP.Customer_Name
		,SUM(SP.Quantity_ordered) AS QTT
		,ROUND(AVG(SP.Sales), 2) AS AVG_SALES 
FROM [dbo].[TB_SUPERSTORE] SP
GROUP BY SP.Customer_Name;


-- COUNT QUANTITY REGISTER BY CUSTOMER AND DATE 
SELECT SP.Customer_Name
		,SP.Order_Date
		,COUNT(*) AS COUNT_REGISTER
FROM TB_SUPERSTORE SP
GROUP BY SP.Customer_Name
		,SP.Order_Date
ORDER BY SP.Order_Date;


-- COUNT QUANTITY REGISTER BY CUSTOMER 
SELECT SP.Customer_Name
		,COUNT(*) AS COUNT_REGISTER
FROM TB_SUPERSTORE SP
GROUP BY SP.Customer_Name
...
-- TAKE CLASSIFIER CUSTOMER WHERE SALES > AVG
SELECT SP.Customer_Name
		,COUNT(*) AS COUNT_REGISTER
		,SUM(SP.Sales) AS SUM_SALES
		,ROUND(AVG(SP.Sales), 2) AS AVG_SALES
		,(SELECT ROUND(AVG(P.Sales), 2) AS AVG_S FROM TB_SUPERSTORE P) AS AVG_TOTAL
		,(CASE WHEN SUM(SP.Sales) > (SELECT ROUND(AVG(P.Sales), 2) AS AVG_S FROM TB_SUPERSTORE P) THEN 'MAIOR' ELSE 'MENOR' END) AS STATUS_1
FROM TB_SUPERSTORE SP
GROUP BY SP.Customer_Name
;
...
---- CORRIGIR ----------------------------------------------------------------------
-- TAKE COUNT CUSTOMER WHERE SALES > AVG 
